  ...
  async function connectToChannel() {
    ...
    gameChannel.on('broadcast', { event: 'PLAYER_LOST' }, ({ payload }) => {
        ...
        if (activePlayers.size <= 1) {
            ...
            endMatch(activePlayers);
        }
    });

    function endMatch(survivors) {
        console.log('endMatch called: survivors=', [...survivors]);
        if (matchTimerInterval) cancelAnimationFrame(matchTimerInterval);
        ...
        let rankings = [];
        // Prioritize the last surviving player as rank 1
        survivors.forEach(winnerName => {
            const winner = players.find(p => p.name === winnerName);
            const winnerScore = winnerName === localPlayerName ? score : winner?.score || 0;
            rankings.push({ name: winnerName, score: winnerScore, rank: 1 });
        });
        // Rank eliminated players starting from rank 2 (or 1 if no survivors)
        const startRank = survivors.size > 0 ? 2 : 1;
        rankings = [
            ...rankings,
            ...eliminatedPlayers
                .sort((a, b) => {
